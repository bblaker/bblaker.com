...
Drop image files here, then reference them from a post's frontmatter or body.
Paths are relative to the `.mdx` file, so from `content/posts/some-entry.mdx` an image
